...
def outPutCalibrated(calibratedResult):
    ...
    pd.DataFrame(_measured_calibrated).to_csv(PATH+'\\Measured_calibrated_VB.csv')
    pd.DataFrame(_demand_calibrated).to_csv(PATH+'\\Demand_calibrated_VB.csv')
    pd.DataFrame(_demand_vairance_calibrated).to_csv(PATH+'\\Demand_vairance_calibrated_VB.csv')
    pd.DataFrame(_Nodal_Pressure_calibrated).to_csv(PATH+'\\Nodal_Pressure_calibrated_VB.csv')
    pd.DataFrame(_Pipe_Flow_calibrated).to_csv(PATH+'\\Pipe_Flow_calibrated_VB.csv')
...
def outPutMeasured(meauredResult):
    ...
    pd.DataFrame(_measured_STD).to_csv(PATH+'\\Measured_VAR_VB.csv')